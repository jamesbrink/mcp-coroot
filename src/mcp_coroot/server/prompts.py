...
1. Call list_alerts to see what is firing now.
2. Call list_incidents with state_filter open for SLO breaches.
3. Call list_applications and note everything at warning or critical.
4. Call list_nodes to check whether any host is down or saturated.
5. Call get_project_status if any of the above look empty, to confirm telemetry
...
1. Call get_incident for {incident_key}: severity, duration, which SLO was
...
3. Call list_deployments over the same window to see whether a rollout coincided.
...
1. Call get_costs and note the applications with the largest gap between
   allocated and used resources, and the nodes with the highest idle costs.
2. Call list_risks: single-instance and spot-only workloads change what is safe
   to shrink.
